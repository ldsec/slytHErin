...
{
   # Display Help
   echo "Syntax: setup.sh [-s|i|n|u|p|h]"
   echo "options:"
...
############################################################
############################################################
echo "Makes sure that you have already the servers in the ssh known hosts"
...
while [ $i -lt $parties ]; do
  id=$((startingId+i))
  echo "connecting to: '${username}:${password}@iccluster${id}.iccluster.epfl.ch'"
  ...
  sshpass -p 1 root@iccluster0"${id}".iccluster.epfl.ch "cd /root/dnn/config; chmod +x go_install.sh; ./go_install.sh"
  i=$((i+1))
done
